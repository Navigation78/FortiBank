    ...
    <div className="min-h-full p-4 sm:p-6 max-w-3xl mx-auto">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-th-muted mt-0.5">{total} total · {unreadCount} unread</p>
        </div>
        <div className="flex items-center gap-2">
          {unreadCount > 0 && (
            <button
              onClick={markAllRead}
              className="text-sm text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-300 px-3 py-1.5 rounded-lg hover:bg-th-hov transition-all duration-150"
            >
              Mark all read
            </button>
          )}
          <button
            onClick={refresh}
            className="flex items-center justify-center w-8 h-8 rounded-lg text-th-muted hover:text-th-txt hover:bg-th-hov transition-all duration-150"
            title="Refresh"
          >
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all duration-150
                ${filter === f.value
                  ...
                  : 'text-th-muted hover:text-th-txt'
                }`}
            >
              {f.label}
            </button>
        ...
        <select
          ...
          className="bg-th-ibg border border-th-ibrd text-th-txt2 text-xs rounded-lg px-3 py-2 outline-none focus:border-green-500/50 transition-colors"
        >
          {TYPES.map(t => (
      ...
      <div className="bg-th-srf border border-th-brd rounded-xl overflow-hidden">
        {loading && notifications.length === 0 ? (
          ...
          <div className="py-16 text-center">
            ...
            />
            <p className="text-th-muted font-medium">No notifications</p>
            <p className="text-th-muted text-sm mt-1 opacity-70">
          ...
          notifications.map(n => {
            ...
            return (
              <div
                key={n.id}
                className={`flex items-start gap-4 px-5 py-4 border-b border-th-brd last:border-0 hover:bg-th-hov/30 transition-colors group
              ...
              >
                ...
                <div className="flex-shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="p-1.5 rounded-lg text-th-muted hover:text-green-600 dark:hover:text-green-400 hover:bg-green-500/10 transition-all duration-150"
                  >
                    <Icon
                      path={n.is_read
