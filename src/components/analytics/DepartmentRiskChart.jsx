...
export default function DepartmentRiskChart({ data = [], loading = false, fetchedAt = null }) {
  function handleDownload() {
    const date = fetchedAt || new Date()
    downloadCSV(
      csvFilename('avg-risk-by-category', date),
      ['Category', 'Avg Risk Score'],
      data.map(d => [d.category, d.avgScore])
    )
  }
  if (loading) {
    return (
  ...
  return (
    <div className="bg-th-srf border border-th-brd rounded-xl p-6">
      <div className="flex items-center justify-between gap-3 mb-4">
        <h3 className="text-th-txt font-semibold">Average Risk Score by Category</h3>
        {data.length > 0 && (
          <button
