      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="bg-th-hov border border-th-brd rounded-xl p-5 animate-pulse">
            <div className="h-3 bg-th-track rounded w-1/3 mb-3" />
            <div className="h-4 bg-th-track rounded w-3/4 mb-2" />
            <div className="h-3 bg-th-track rounded w-full mb-4" />
            <div className="h-1.5 bg-th-track rounded" />
          </div>
        ))}
      </div>
    )
    ...
    <div>
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                filter === f.key
                  ...
                  : 'bg-th-hov text-th-txt2 hover:text-th-txt hover:bg-th-act'
              }`}
            >
              {f.label}
              <span className={`text-xs px-1.5 py-0.5 rounded-md ${
                filter === f.key ? 'bg-blue-500/40' : 'bg-th-act'
              }`}>
                {count}
              </span>
            </button>
      ...
      {filtered.length > 0 ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-th-txt2 font-medium">No modules found</p>
