...
export default function QuizCard({
  ...
  function getOptionStyle(optionId) {
    ...
    if (showResult) {
      ...
      return 'border-th-brd text-th-muted'
    }

    if (isSelected)
      return 'border-blue-500 bg-blue-500/10 text-th-txt'

    return 'border-th-brd hover:border-blue-500/50 text-th-txt2 hover:text-th-txt'
  }
  ...
  function getOptionIcon(optionId) {
    ...
    if (isMultiSelect) {
      return (
        <span className={`w-5 h-5 rounded flex items-center justify-center border flex-shrink-0 ${
          isSelected ? 'bg-blue-500 border-blue-500' : 'border-th-ibrd'
        }`}>
          {isSelected && (
    ...
    return (
      <span className={`w-5 h-5 rounded-full border flex-shrink-0 flex items-center justify-center ${
        isSelected ? 'border-blue-500' : 'border-th-ibrd'
      }`}>
  ...
  return (
    <div className="bg-th-srf border border-th-brd rounded-xl p-6 space-y-5">
      {/* Question */}
      <div>
        {isMultiSelect && (
        ...
        <p className="text-th-txt font-medium leading-relaxed">
      ...
      {showResult && question.explanation && (
        <div className="bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 rounded-lg px-4 py-3">
          <p className="text-blue-700 dark:text-blue-400 text-xs font-semibold mb-1">Explanation</p>
          <p className="text-th-txt2 text-sm">{question.explanation}</p>
        </div>
      )}
    </div>
  )
